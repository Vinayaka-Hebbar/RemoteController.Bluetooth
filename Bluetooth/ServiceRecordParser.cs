...
(byte[] buffer, int offset, int length, out int readLength);
        ...
        public ServiceRecord Parse(byte[] buffer, int offset, int length)
        {
            ...
            { throw new ArgumentNullException("buffer"); }
            if (length <= 0)
            { throw new_ArgumentOutOfRangeException("buffer", ErrorMsgServiceRecordBytesZeroLength); }
            if (offset < 0)
    ...
    */
            ...
            )
        {
            ...
            while (curOffset < offset + length && curContentLength > 0)
            {
                ...
                T
#endif
                    child = itemParser(buffer, curOffset, curContentLength, out int readLength);
                children.Add(child);
                curOffset += readLength;
                curContentLength -= readLength;
            }//while
        ...
        public static byte[][] SplitSearchAttributeResult(byte[] multiRecord)
        {
            ...
            while (curLength > 0)
            {
                ...
                //
                int elementLengthInner = GetElementLength(buffer, curOffset, curLength, out int contentOffsetInner, out int contentLengthInner);
...
#pragma warning restore 618
    ...
    */
            ...
            out int contentOffset, out int contentLength)
        {
            ...
            //
            SplitHeaderByte(buffer[index], out ElementTypeDescriptor etd, out SizeIndex si);
            if (etd == ElementTypeDescriptor.Nil)
            {
            ...
            else
            {
                switch (si)
                {
                    ...
                    case SizeIndex.AdditionalUInt16:
                        ...
                        contentOffset = 3;
                        ushort length16 = ReadFieldUInt16(buffer, index + 1, length - 1);
                    ...
                    default:
                        ...
                        contentOffset = 5;
                        uint length32 = ReadFieldUInt32(buffer, index + 1, length - 1);
                        return FixupLength(length32, contentOffset, out contentLength, index);
                }
            }
        }
        private static int FixupLength(uint contentLength, int contentOffsetAlsoHeaderBytesLength, out int outContentLength, int index)
        {
            long fullLength = contentLength + contentOffsetAlsoHeaderBytesLength;
            if (fullLength > int.MaxValue)
            {
        ...
        private static ushort ReadFieldUInt16(bool networkOrder, byte[] bytes, int index, int length)
        {
            ...
            ushort value16 = unchecked((ushort)value16Signed);
            return value16;
        }
        ...
        private static uint ReadFieldUInt32(bool networkOrder, byte[] bytes, int index, int length)
        {
            ...
            uint value32 = unchecked((uint)value32Signed);
            return value32;
        }
        ...
        private static ulong ReadFieldUInt64(bool networkOrder, byte[] bytes, int index, int length)
        {
            ...
            long value64TypeToSuitHostToNetworkOrder = BitConverter.ToInt64(bytes, index);
            long value64Signed;
            if (networkOrder)
            ...
            ulong value64 = unchecked((ulong)value64Signed);
            return value64;
        }
        ...
        {
            uint etdRaw = (uint)headerByte >> ElementTypeDescriptorOffset;
