...
namespace RemoteController.Bluetooth
{
    ...
    public sealed class ServiceRecord : IEnumerable<ServiceAttribute>
    {
        ...
        {
            return TryGetAttributeById(id, out ServiceAttribute tmp);
        }
        ...
        /// </exception>
        public ServiceAttribute GetAttributeById(ushort id)
        {
        ...
        private bool TryGetAttributeById(ushort id, out ServiceAttribute attribute)
        {
...
#endif
        public IReadOnlyList<ushort> AttributeIds
        {
            get
            {
        ...
        public static ushort CreateLanguageBasedAttributeId(ushort id, ushort baseId)
        {
            ushort offset = baseId;
            ushort actualId = checked((ushort)(id + offset));
        ...
        public LanguageBaseItem GetPrimaryLanguageBaseItem()
        {
            ...
            const short PrimaryBaseId = 0x0100;
            foreach (LanguageBaseItem item in list)
            {
                if (item.AttributeIdBase == PrimaryBaseId)
                { return item; }
            }//for
            return null;
        }
        ...
        internal sealed class ServiceRecordEnumerator : IEnumerator<ServiceAttribute>
        {
            ...
            public ServiceAttribute Current //x
            {
                get
                {
                    ...
                    if (m_currentIndex >= m_record.Count)
                    { throw new InvalidOperationException(); }
