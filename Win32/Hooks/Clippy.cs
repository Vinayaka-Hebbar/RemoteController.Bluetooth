...
namespace RemoteController.Win32.Hooks
{
    public static class Clippy
    {
        ...
        {
            bool isAscii = (message != null && (message == Encoding.ASCII.GetString(Encoding.ASCII.GetBytes(message))));
            if (isAscii)
            {
            ...
            try
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        uint characters = (uint)message.Length;
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    bool ignore = GlobalUnlock(target);
                                }
                        ...
                        finally
                        {
                            if (hGlobal != IntPtr.Zero)
                            {
                                IntPtr ignore = GlobalFree(hGlobal);
                            }
                        }
                    }
                    finally
