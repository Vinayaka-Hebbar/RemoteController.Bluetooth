...
namespace RemoteController.Win32
{
    ...
    /// <see cref="BluetoothAuthenticationEventArgs"/> it includes
    /// an example, 
    /// also see the documentation on each of this class&#x2019;s properties.
    /// </para>
    /// </remarks>
    public sealed class BluetoothAuthenticationEventArgs : EventArgs
    {
        ...
        /// </summary>
        public BluetoothAuthenticationEventArgs()
        { }
...
#if DEBUG
                    string zerosThrees = 0.ToString(SixDigitsFormatString);
                    Debug.Assert(zerosThrees.Length == 6);
#endif
                    string twoThrees = NumberOrPasskey.Value
        ...
        public void ConfirmOob(byte[] c, byte[] r)
        {
            ...
            if (!(c == null || c.Length == 16))
                throw new ArgumentException(Must16bytes, "c");
            if (!(r == null || r.Length == 16))
                throw new ArgumentException(Must16bytes, "r");
            //
            _oobC = c;
            _oobR = r;
        ...
        public bool CannotSendAnotherResponse
        {
            get
            {
                return m_errorCode == BluetoothAuthentication.NativeErrorDeviceNotConnected;
            }
        }
